...
for r in range(rodadas):
    ...
    beta_hat_mqo = np.linalg.pinv(X_treino_.T@X_treino_)@X_treino_.T@y_treino
    #Predição
...
resultados = {
    ...
    "MQO regularizado (λ=0.25)": rss_ridge_map[0.25],
    "MQO regularizado (λ=0.50)": rss_ridge_map[0.50],
    "MQO regularizado (λ=0.75)": rss_ridge_map[0.75],
    "MQO regularizado (λ=1.00)": rss_ridge_map[1.00],
}
...
for nome, lista_rss in resultados.items():
    ...
    print(f"{nome:<30} | {media:15.4f} | {desvio_padrao:15.4f} | {maior_valor:15.4f} | {menor_valor:15.4f}")import numpy as np
import matplotlib.pyplot as plt
from numpy.linalg import pinv
